class ConversationFinder
  ...
  def filter_by_assignee_type
    ...
    when 'agent_bot'
      @conversations = @conversations.where.not(assignee_agent_bot_id: nil)
    when 'collaborators'
      @conversations = @conversations.where.not(assignee_id: nil).where.not(assignee_id: current_user.id)
    end
    @conversations
  end
